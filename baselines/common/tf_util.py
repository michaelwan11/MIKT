...
def load_variables_for_transfer(variables=None, loaded_params=None, sess=None,
                                transfer=None, transfer_env=None):
    print('Loading variables for transfer')
    restores = []
    gnn_node_option = 'nG,nB'
    root_connection_option = 'nN,Rb,uE'
    ...
    '''
    gnn_option_list = [gnn_node_option,
...
def load_variables(load_path, variables=None, sess=None, transfer=None,
                   transfer_env=None):
    sess = sess or get_session()
    variables = variables or tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES)
    ...
    if transfer:
        load_variables_for_transfer(variables, loaded_params, sess, transfer,
                                    transfer_env)
        return
    restores = []
    if isinstance(loaded_params, list):
    ...
    print('loading variables from', load_path)
    sess.run(restores)
